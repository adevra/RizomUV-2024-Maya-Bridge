...
def find_rizomuv_installations():
    ...
    if system == "Windows":
        ...
        if local_app_data:
             search_patterns.append(os.path.join(local_app_data, r"Programs\Rizom Lab\RizomUV*\rizomuv.exe"))
...
def browse_manually(parent_window):
    ...
    if system == "Windows":
        ...
        path_tuple = QtWidgets.QFileDialog.getOpenFileName(parent_window, "Select RizomUV Executable", default_path_dir, qfile_filter)
        ...
        qfile_filter = f"{file_filter} (*);;All Files (*)"
        path_tuple = QtWidgets.QFileDialog.getOpenFileName(parent_window, "Select RizomUV Executable/AppImage", default_path_dir, qfile_filter)
        if path_tuple and path_tuple[0]:
             selected_path = path_tuple[0]
    if selected_path:
...
class SelectRizomDialog(QtWidgets.QDialog):
    def __init__(self, paths, parent=None):
        ...
        label = QtWidgets.QLabel("Multiple RizomUV installations detected.\nPlease select the version to use:")
...
def onMayaDroppedPythonFile(*args):
    ...
    try:
        ...
        if num_found == 0:
             logging.warning("Could not automatically detect RizomUV. Please browse manually.")
             cmds.confirmDialog(
             ...
             if not QT_AVAILABLE:
                  ...
                  logging.info(f"Multiple ({num_found}) RizomUV installations detected. Showing selection dialog.")
                  selected_path_from_dialog, needs_browse = SelectRizomDialog.get_selection(detected_paths, maya_window)
                  if needs_browse:
                      rizomPath = browse_manually(maya_window)
                      if not rizomPath:
                           raise RuntimeError("Manual selection cancelled or failed.")
        ...
        if system == "Darwin":
            if not rizomPath.endswith(".app") or not os.path.isdir(rizomPath):
                raise RuntimeError(f"Invalid selection on macOS. Path must be an application bundle ending in .app. Selected: {rizomPath}")
        elif system == "Windows" and not rizomPath.lower().endswith(".exe"):
            raise RuntimeError(f"Invalid selection on Windows. Path must be an executable file ending in .exe. Selected: {rizomPath}")
        elif system == "Linux":
            if not os.path.isfile(rizomPath) or not os.access(rizomPath, os.X_OK):
                 raise RuntimeError(f"Selected path on Linux is not an executable file (or AppImage). Check permissions. Selected: {rizomPath}")
        ...
        if placeholder in script_content:
            ...
            raise RuntimeError("PATH_DEFAULTS dictionary placeholder not found in the main script. Cannot set path.")
        ...
        if not button_exists:
            ...
            cmds.shelfButton(
                ...
                annotation="RizomUV Bridge", label=button_label,
        ...
        logging.error(f"Runtime error: {e}")
         ...
         try:
             cmds.confirmDialog(message=f"Failed to import Qt bindings. Cannot show UI elements.\n{e}", icon="critical", title="ERROR")
